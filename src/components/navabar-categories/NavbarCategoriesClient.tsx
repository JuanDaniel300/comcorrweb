...
export default function NavbarCategoriesClient({ categorias }: { categorias: any }) {
            ...
            <div className="container px-20 text-white mx-auto flex justify-between py-3">
                {(categorias).map((category: any) => (
                    <div
                        key={category.nombre}
                        className="relative"
                        ref={(el) => { menuRefs.current[category.nombre] = el; }}
                    >
                        {/* Categoría principal */}
                        <div
                        ...
                        >
                            {category.lineas.map((sub: any, index: any) => (
                                <Link
                                    href={`/categoria/${generarSlug(category.nombre)}/${generarSlug(sub)}`}
                                    key={index}
                                    onClick={() => handleToggle(category.nombre)}
                                >
