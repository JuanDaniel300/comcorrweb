...
export default function ProductGrid({ products, title, Breadcrumb }: { products: any[], title: string, Breadcrumb: any[] }) {
    ...
    const filteredProducts = useMemo(() => {
        return products
            .filter((product) => {
                // Filtro de precio
                if (product.precio1 < priceRange[0] || product.precio1 > priceRange[1]) {
                    return false;
                }
                ...
                <div className="flex flex-col md:flex-row gap-6">
                    ...
                    >
                        <Card className="border-claro2 bg-claro100">
                            <CardContent className="px-6">
                                <h2 className="text-xl font-bold text-primario mb-6">Filtros</h2>

                                {/* Sort Options */}
                                <div className="mb-6">
                                    <h3 className="font-semibold mb-3 text-oscuro1">Ordenar por</h3>
                                    <RadioGroup value={sortOption} onValueChange={setSortOption}>
                                        <div className="space-y-2">
                                            <div className="flex items-center space-x-2">
                                                <RadioGroupItem value="lowToHigh" id="lowToHigh" className="text-primario" />
                                                <Label htmlFor="lowToHigh" className="text-oscuro2">
                                                    Precio (menor a mayor)
                                                </Label>
                                            </div>
                                            <div className="flex items-center space-x-2">
                                                <RadioGroupItem value="highToLow" id="highToLow" className="text-primario" />
                                                <Label htmlFor="highToLow" className="text-oscuro2">
                                                    Precio (mayor a menor)
                                                </Label>
                                            </div>
                                            <div className="flex items-center space-x-2">
                                                <RadioGroupItem value="bestSellers" id="bestSellers" className="text-primario" />
                                                <Label htmlFor="bestSellers" className="text-oscuro2">
                                                    Más vendidos
                                                </Label>
                                            </div>
                                            <div className="flex items-center space-x-2">
                                                <RadioGroupItem value="newest" id="newest" className="text-primario" />
                                ...
                                <div className="mb-6">
                                    ...
                                    <div className="text-center mb-4">
                                        <span className="text-primario font-medium">{formatCurrency(priceRange[0])}</span>
                                        <span className="text-oscuro2"> - </span>
                                        <span className="text-primario font-medium">{formatCurrency(priceRange[1])}</span>
                                    </div>
                                ...
                                {/* Availability */}
                                <div className="mb-6">
                                    <h3 className="font-semibold mb-3 text-oscuro1">Disponibilidad</h3>
                                    <div className="flex items-center space-x-2">
                                        <Checkbox
                                            id="inStock"
                                            checked={inStockOnly}
                                        ...
                                        {BRANDS.map((brand) => (
                                            <div key={brand} className="flex items-center space-x-2">
                                                <Checkbox
                                                    ...
                                                    className="text-primario border-claro2"
                                                />
                                                <Label htmlFor={`brand-${brand}`} className="text-oscuro2">
                                                    {brand}
                                                </Label>
                                            </div>
                                        ))}
                                ...
                                {/* Discounts */}
                                <div className="mb-6">
                                    <h3 className="font-semibold mb-3 text-oscuro1">Descuentos o Promociones</h3>
                                    <div className="space-y-2">
                                        {PROMOTIONS.map((promo) => (
                                            <div key={promo} className="flex items-center space-x-2">
                                                <Checkbox
                                                    ...
                                                    onCheckedChange={(checked) => handlePromotionChange(promo, checked as boolean)}
                                                    className="text-primario border-claro2"
                                                />
                                                <Label htmlFor={`promo-${promo}`} className="text-oscuro2">
                                                    {promo}
                                                </Label>
                                            </div>
                                        ))}
                                        ...
                                        {TYPES.map((type) => (
                                            <div key={type} className="flex items-center space-x-2">
                                                <Checkbox
                                                    ...
                                                    className="text-primario border-claro2"
                                                />
                                                <Label htmlFor={`type-${type}`} className="text-oscuro2">
                                                    {type}
                                                </Label>
                                            </div>
                                        ))}
                    ...
                    <div className="flex-1">
                        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="mb-6">
                            <h1 className="text-2xl font-bold text-primario">{capitalize(title)}</h1>
                            <p className="text-oscuro3">{loading ? "Cargando..." : `${filteredProducts.length} resultados`}</p>
                        </motion.div>
                        ...
                        {loading ? (
                            ...
                            >
                                {filteredProducts.map((product, index) => (
                                    <ProductCard keyIndex={index} product={product} />
                                ))}
                            </motion.div>
                        ) : (
                            <div className="text-center py-12 bg-claro100 rounded-lg border border-claro2">
                                <h3 className="text-lg font-medium mb-2 text-oscuro1">No se encontraron productos</h3>
                                <p className="text-oscuro3 mb-4">No hay productos que coincidan con los filtros seleccionados.</p>
                                <Button
